...
@section('content')
<div class="mb-6">
    <a href="{{ route('categories.index') }}" class="inline-flex items-center gap-2 text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 transition-colors">
    ...
    <div class="bg-gradient-to-r from-green-500 to-green-600 dark:from-green-600 dark:to-green-700 px-8 py-6">
        <div class="flex items-start justify-between">
            <div class="flex-1">
                ...
                <p class="text-3xl font-bold text-green-100">${{ number_format($category->price, 2) }}</p>
            </div>
            <div class="flex gap-2">
                <a href="{{ route('categories.edit', $category) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-white/20 hover:bg-white/30 text-white rounded-lg transition-colors backdrop-blur-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="space-y-6">
            @if($category->description)
                <div>
                    ...
                    <p class="text-lg text-[#1b1b18] dark:text-[#EDEDEC] leading-relaxed">{{ $category->description }}</p>
                </div>
            @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-[#e3e3e0] dark:border-[#3E3E3A]">
                <div>
                    ...
                    <p class="text-lg text-[#1b1b18] dark:text-[#EDEDEC]">{{ $category->created_at->format('F j, Y g:i A') }}</p>
                </div>
                <div>
                    <h3 class="text-sm font-semibold text-[#706f6c] dark:text-[#A1A09A] mb-1 uppercase tracking-wide">Updated At</h3>
                    <p class="text-lg text-[#1b1b18] dark:text-[#EDEDEC]">{{ $category->updated_at->format('F j, Y g:i A') }}</p>
                </div>
            </div>
        </div>
    </div>
